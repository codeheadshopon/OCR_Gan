...
class DCGAN(object):
    def __init__(self, img_rows=128, img_cols=32, channel=1):
    ...
    def generator(self):
        ...
        dim2 = 16
        # Input = 164 + 1 = 165
        G.add(Dense(dim1 * dim2 * depth, input_dim=165))
        G.add(BatchNormalization(momentum=0.9))
        G.add(Activation('relu'))
        G.add(Reshape((dim1, dim2, depth)))
...
class OCR_DCGAN(object):
    def __init__(self):
        ...
        self.x_train = dataset_load('./ocrgansmall.pkl.gz')
    ...
    def train(self, train_steps=2000, batch_size=256, save_interval=0):
        noise_input = None
        if save_interval>0:
            noise_input = np.random.uniform(-1.0, 1.0, size=[16, 165])
        for i in range(train_steps):
            ...
            print("H1",images_train[0].shape)
            # print("H2".images_fake[0].shape)
            x = np.concatenate((images_train, images_fake)) # Merge the original and fake images
